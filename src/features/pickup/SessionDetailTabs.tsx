...
function SessionDetailTabs({
  ...
  // async function uploadToHetzner(imageDataUrl: string): Promise<string> {
  ...
  //   return data.url;
  // }
  // async function updateDropSessionUrl(id: string, photoUrl: string) {
  //   await fetch(`/api/dropsession/${id}`, {
  //     method: "PATCH",
  //     headers: { "Content-Type": "application/json" },
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Capture Guardian Photo</DialogTitle>
          </DialogHeader>
          {!image ? (
            <>
            ...
            <>
              ...
              <Button
                onClick={async () => {
                  // const uploadedUrl = await uploadToHetzner(image); // See next step
                  // await updateDropSessionUrl(dropSessionId, uploadedUrl); // See next step
                  setOpen(false);
                }}
              >
                Upload
      ...
      <Tabs defaultValue="details" className="flex-1 flex flex-col">
        <TabsList className="grid grid-cols-2 bg-muted rounded-none px-2 pt-2 bg-transparent">
          <TabsTrigger
            value="details"
          ...
          <Card className="mb-4 border-none shadow-sm">
            <CardContent className="p-0">
              <div className="p-4 flex items-center space-x-4 bg-white dark:bg-zinc-950/80 rounded-t-lg">
                <Button
                  variant="ghost"
                  className="p-0 h-14 w-14 rounded-full"
